...
class _shopPageState extends State<shopPage> with WidgetsBindingObserver{
  ...
  void _initInAppPurchase() async {
    ...
    if (isAvailable) {
      final Set<String> productIds = {'25energy'};
  ...
  void _initInAppPurchase3() async {
    final bool isAvailable = await InAppPurchase.instance.isAvailable();
    if (isAvailable) {
      final Set<String> productIds = {'candy200'}; // doğru ürün tanımlayıcısını buraya ekleyin
      final ProductDetailsResponse productDetailsResponse = await InAppPurchase.instance.queryProductDetails(productIds);
      if (productDetailsResponse.notFoundIDs.isNotEmpty) {
        print('Product not found');
        return; // metodu sonlandırın
      ...
      final List<ProductDetails> products = productDetailsResponse.productDetails;
      products.forEach((product) {
        if (product.id == 'candy200') {
          productDetails3 = product;
        }
      });
      final Stream purchaseUpdates = InAppPurchase.instance.purchaseStream;
      purchaseUpdates.listen((purchaseDetailsList) {
  ...
  void _initInAppPurchase4() async {
    final bool isAvailable = await InAppPurchase.instance.isAvailable();
    if (isAvailable) {
      final Set<String> productIds = {'candy500'}; // doğru ürün tanımlayıcısını buraya ekleyin
      final ProductDetailsResponse productDetailsResponse = await InAppPurchase.instance.queryProductDetails(productIds);
      if (productDetailsResponse.notFoundIDs.isNotEmpty) {
        print('Product not found');
        return; // metodu sonlandırın
      ...
      final List<ProductDetails> products = productDetailsResponse.productDetails;
      products.forEach((product) {
        if (product.id == 'candy500') {
          productDetails4 = product;
        }
      });
      final Stream purchaseUpdates = InAppPurchase.instance.purchaseStream;
      purchaseUpdates.listen((purchaseDetailsList) {
  ...
  void _listenToPurchaseUpdated(List<PurchaseDetails> purchaseDetailsList) {
    purchaseDetailsList.forEach((purchaseDetails) async {
      if (purchaseDetails.status == PurchaseStatus.purchased) {
        ...
        if (isValid) {
          if (purchaseDetails.productID == '25energy') {
            incrementsEnergy2();
          }
          else if (purchaseDetails.productID == 'candy25') {
            incrementsCandy25();
          }
          else if (purchaseDetails.productID == 'candy200') {
            incrementsCandy200();
          }
          else if (purchaseDetails.productID == 'candy500') {
            incrementsCandy500();
          }
        }
        else {
  ...
  Future<void> _buyProduct3() async {
    if (productDetails3 != null) {
      final PurchaseParam purchaseParam =
      PurchaseParam(productDetails: productDetails3!);
      await InAppPurchase.instance.buyNonConsumable(
          purchaseParam: purchaseParam);
    } else {
      print('Product not found');
  ...
  Future<void> _buyProduct4() async {
    if (productDetails4 != null) {
      final PurchaseParam purchaseParam =
      PurchaseParam(productDetails: productDetails4!);
      await InAppPurchase.instance.buyNonConsumable(
          purchaseParam: purchaseParam);
    } else {
      print('Product not found');
  ...
  Future<bool> _verifyPurchase(PurchaseDetails purchaseDetails) async {
    if (purchaseDetails.productID == '25energy') {
      // Satın alma işlemi doğrulama işlemi başarılı ise, true değeri döndürün.
      return true;
    }else if (purchaseDetails.productID == 'candy25') {
      // Satın alma işlemi doğrulama işlemi başarılı ise, true değeri döndürün.
      return true;
                        ...
                        children: [
                          ...
                          Row(
                            ...
                            children: [
                              ...
                              Image.asset('assets/shopPage/energyIcon.png', width: 30, height: 30,),
                              Text('1dollar'.tr, style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold),),
                            ],
                          ),
                        ],
                      ),
                        ...
                        children: [
                          ...
                          Row(
                            ...
                            children: [
                              ...
                              Image.asset('assets/shopPage/starIcon.png', width: 30, height: 30,),
                              Text('5dollar'.tr, style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold),),
                            ],
                          ),
                        ],
                      ),
                        ...
                        children: [
                          ...
                          Row(
                            ...
                            children: [
                              ...
                              Image.asset('assets/shopPage/starIcon.png', width: 30,height: 30,),
                              Text('10dollar'.tr, style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold),),
                            ],
                          ),
                        ],
                      ),
